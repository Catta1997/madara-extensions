...
//#endregion
//#region src/Manga3asq/parsers.ts
	var Manga3asqParser = class extends MadaraParser {
		constructor(..._args) {
			super(..._args);
			_defineProperty(this, "parseDate", (date) => {
				date = date.toUpperCase().trim();
				if (date.includes("قبل ساعة") || date.includes("الان")) return /* @__PURE__ */ new Date();
				if (date.includes("يومين")) return /* @__PURE__ */ new Date(Date.now() - 2 * 864e5);
				const timeUnits = {
					سنة: 31556952e3,
					سنوات: 31556952e3,
					شهر: 2592e6,
				...
				const match = date.match(/(\d+)\s*(سنة|سنوات|شهر|شهور|اسبوع|اسبوعين|اسابيع|يوم|ايام|ساعة|ساعات|دقيقة|دقائق|ثانية|ثواني|ثانيا)/);
				if (match) {
					const [, numStr, unit] = match;
					const number = Number(numStr);
					return new Date(Date.now() - number * timeUnits[unit]);
...
//#endregion
//#region src/Manga3asq/pbconfig.ts
...
//#region src/Manga3asq/main.ts
	...
	const Manga3asq = new Manga3asqExtension();
...
//# sourceMappingURL=Manga3asq.js.map