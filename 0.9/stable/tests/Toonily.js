...
//#endregion
//#region src/generic/forms.ts
	function toBoolean(value) {
		if (value === true || value === "true" || value === 1) return true;
		if (value === false || value === "false" || value === 0) return false;
	}
	function getUsePostIds(sourcePreference) {
		if (sourcePreference === false) return false;
	...
	var MadaraSettings = class extends Form {
		constructor(source) {
			super();
		...
		getSections() {
			return [Section(`${this.source.name} Settings`.replaceAll(" ", ""), [ToggleRow("postIds", {
				title: "Use Post IDs",
				value: getUsePostIds(),
				onValueChange: Application.Selector(this, "usePostIdsChange"),
				...
				onValueChange: Application.Selector(this, "useHQThumbnailsChange"),
				subtitle: "Enabling will make the sources use more bandwith"
			})]), Section("second", [ButtonRow("resetPath", {
				title: "Reset Stored Directory Path",
				onSelect: Application.Selector(this, "resetDirectoryPath")
...
//#region src/generic/network.ts
	var MadaraInterceptor = class extends PaperbackInterceptor {
		...
		async interceptRequest(request) {
			if (!request.url.includes("#directoryRequest")) {
				if (!this.promise) {
					if (!request.url.includes("#directoryRequest")) {
						if (!this.promise) this.promise = this.source.getDirectoryPath();
						try {
			...
			request.headers = {
				...
				...(request.url.includes("wordpress.com") || request.url.includes("wp.com")) && { Accept: "image/avif,image/webp,*/*" }
			};
			request.cookies = {
				...request.cookies,
				"toonily-mature": "1",
		...
		async interceptResponse(request, response, data) {
			...
			if (response.status !== 200) {
				if (response.status === 404 && request.url.includes(this.source.searchPagePathName)) return data;
...
//#region src/generic/utils.ts
	...
	function md5Bytes(data) {
		return hexToBytes(Application.crypto_md5Hash(data.buffer));
	}
...
//#region src/generic/parsers.ts
	var MadaraParser = class {
		...
		async parseMangaDetails($, mangaId, source) {
			const title = Application.decodeHTMLEntities($("div.post-title h1, div#manga-title h1").children().remove().end().text().trim());
			const secondaryTitleBox = $("h5:contains(Alternative), h5:contains(Alt Name(s))").parent().next();
			const secondaryTitles = [];
			for (const title of secondaryTitleBox.text().trim().split(/\s*[/,;]\s*/)) {
				if (title == "" || !title) continue;
				secondaryTitles.push(Application.decodeHTMLEntities(title?.trim()));
			}
			const author = Application.decodeHTMLEntities($("div.author-content").first().text().replace("\\n", "").trim()).replace("Updating", "");
			const artist = Application.decodeHTMLEntities($("div.artist-content").first().text().replace("\\n", "").trim()).replace("Updating", "");
			const synopsis = Application.decodeHTMLEntities($("div.description-summary, div.summary-container, div.manga-excerpt").first().text()).replace("Show more", "").trim();
			const shareUrl = `${source.domain}/${await source.getDirectoryPath()}/${mangaId}`;
			const rating = (Number($("#averagerate").text().trim()) || 0) / 10;
			...
			if ($("span.manga-title-badges.custom.adult").length) contentRating = ContentRating$1.ADULT;
			const genres = [];
			for (const obj of $("div.genres-content a").toArray()) {
				const title = $(obj).text();
				const id = this.idCleaner($(obj).attr("href") ?? "");
				if (!title || !id) continue;
				if (["adult", "mature"].includes(title.toLowerCase())) contentRating = ContentRating$1.ADULT;
				genres.push({
					title,
					id
				});
		...
		parseChapterList($, sourceManga, source) {
			...
			for (const obj of nodeArray) {
				...
				if (typeof timeSelector !== "undefined") mangaTime = this.parseDate(timeSelector ?? "");
				else mangaTime = this.parseDate($("span.chapter-release-date > i", obj).text().trim());
				if (!mangaTime.getTime()) mangaTime = /* @__PURE__ */ new Date();
				if (!id || typeof id === "undefined" || id === "#") {
					console.log(`Could not parse out ID when getting chapters for mangaId:${sourceManga.mangaId} parsedId: ${id}`);
					continue;
				}
				chapters.push({
					sourceManga,
		...
		async parseChapterDetails($, chapter, selector, source) {
			const pages = [];
		...
		async parseProtectedChapterDetails($, chapter, selector, source) {
			if (!$(selector).length) return this.parseChapterDetails($, chapter, selector, source);
			const variables = extractVariableValues($(selector).get()[0].children[0].data);
			if (!("chapter_data" in variables) || !("wpmangaprotectornonce" in variables)) throw new Error(`Could not parse page for mangaId:${chapter.sourceManga.mangaId} chapterId:${chapter.chapterId}. Reason: Lacks sufficient data`);
			const chapterList = await decryptData(variables["chapter_data"], variables["wpmangaprotectornonce"]);
			const pages = [];
		...
		async parseSearchResults($, source) {
			...
			for (const obj of $(source.searchMangaSelector).toArray()) {
				...
				const image = encodeURI(await this.getImageSrc($("img", obj), source));
				const rating = $(source.searchRatingSelector, obj).text().trim() ?? "";
				const subtitle = $("span.font-meta.chapter", obj).text().trim();
				results.push({
					slug,
					image,
					title: Application.decodeHTMLEntities(title),
					subtitle: Application.decodeHTMLEntities(subtitle ? `${subtitle} | ⭐${rating}` : `⭐${rating}`)
				});
			}
			return results;
		}
		parseDirectoryPath($, source) {
			const searchResultPath = ($("a", $(source.searchMangaSelector).first()).attr("href") ?? "").replace(/\/$/, "").split("/").slice(-2).shift() ?? "";
			if (searchResultPath) return searchResultPath;
			const pathSegment = (($("meta[property=\"og:url\"]").attr("content") ?? "").replace(source.domain, "").split("/")[1] ?? "").trim();
			if (pathSegment) return pathSegment;
			return "manga";
		}
		async getImageSrc(imageObj, source) {
			let image;
			if (typeof imageObj?.attr("data-src") != "undefined" && imageObj?.attr("data-src") != "") image = imageObj?.attr("data-src");
			else if (typeof imageObj?.attr("data-lazy-src") != "undefined" && imageObj?.attr("data-lazy-src") != "") image = imageObj?.attr("data-lazy-src");
			else if (typeof imageObj?.attr("srcset") != "undefined" && imageObj?.attr("srcset") != "") image = imageObj?.attr("srcset")?.split(" ")[0] ?? "";
			else if (typeof imageObj?.attr("src") != "undefined" && imageObj?.attr("src") != "") image = imageObj?.attr("src");
			else if (typeof imageObj?.attr("data-cfsrc") != "undefined" && imageObj?.attr("data-cfsrc") != "") image = imageObj?.attr("data-cfsrc");
			else image = "";
			if (getUseHQThumbnails()) image = image?.replace(/-\d+x\d+/g, "");
	...
	var MadaraGeneric = class {
		...
		*/
		constructor(params) {
			...
			_defineProperty(this, "usePostIds", void 0);
			_defineProperty(this, "searchPagePathName", void 0);
			_defineProperty(this, "searchMangaSelector", void 0);
			_defineProperty(this, "searchRatingSelector", void 0);
			_defineProperty(this, "hasProtectedChapters", void 0);
			_defineProperty(this, "protectedChapterDataSelector", void 0);
			_defineProperty(this, "chapterEndpoint", void 0);
			_defineProperty(this, "chapterDetailsSelector", void 0);
			_defineProperty(this, "bypassPage", void 0);
			_defineProperty(this, "directoryPath", void 0);
			_defineProperty(this, "useListParameter", void 0);
		...
		async getChapters(sourceManga) {
			...
			switch (this.chapterEndpoint) {
				...
				case 3:
					...
					break;
				default: throw new Error("Invalid chapter endpoint!");
			}
		...
		async getChapterDetails(chapter) {
			...
			const url = new URL$1(this.domain).addPathComponent("temp_dirpath");
			url.addPathComponent(mangaId.slug);
			url.addPathComponent(chapterId);
			if (this.useListParameter) url.setQueryItem("style", "list");
			const [_response, buffer] = await Application.scheduleRequest({
				url: url.toString(),
				method: "GET"
			});
			const $ = load(Application.arrayBufferToUTF8String(buffer));
			if (this.hasProtectedChapters) return this.parser.parseProtectedChapterDetails($, chapter, this.protectedChapterDataSelector, this);
		...
		async getSearchResults(query, metadata) {
			...
			if (_response.status === 404) return {
				items: [],
				metadata: void 0
			};
			const $ = load(Application.arrayBufferToUTF8String(buffer));
			...
			const items = [];
			for (const result of results) if (getUsePostIds(this.usePostIds)) items.push({
		...
		constructSearchRequest(page, query) {
			const urlBuilder = new URL$1(this.domain).addPathComponent(this.searchPagePathName).addPathComponent(page.toString()).setQueryItem("s", this.sanitizeQuery(query?.title ?? "")).setQueryItem("post_type", "wp-manga");
			const genreFilters = Object.keys((query.metadata ?? []).find((x) => x.id === "genres")?.value ?? {});
		...
		async getPostAndSlug(mangaId) {
			...
			if (!isPostId) {
				if (getUsePostIds()) {
					...
					if (!postId) postId = await this.convertSlugToPostId(slugInput);
					slug = slugInput;
				}
			} else {
				const postIdInput = Number(mangaId);
				slug = Application.getState(postIdInput.toString());
				if (!slug) slug = (await this.convertPostIdToSlug(postIdInput)).slug;
				postId = postIdInput;
			}
			if (getUsePostIds()) {
		...
		async convertPostIdToSlug(postId) {
			...
			if (!parseURL.includes(this.domain)) parseURL = $("link[rel=\"canonical\"]").attr("href") ?? "";
			if (!parseURL.includes(this.domain)) throw new Error(`Unable to parse slug for postId: ${postId}!`);
			const URLSplit = parseURL.replace(/\/$/, "").split("/");
			const slug = URLSplit.slice(-1).pop() ?? "";
			const path = URLSplit.slice(-2).shift() ?? "";
			if (!slug) throw new Error(`Unable to fetch slug for this item! postId: ${postId}`);
			return {
				path,
				slug
			};
		...
		async convertSlugToPostId(slug) {
			...
			if (!isNaN(postIdMatch)) return postIdMatch;
			const [, buffer] = await Application.scheduleRequest({
				url: `${this.domain}/temp_dirpath/${slug}`,
				method: "GET"
			});
			...
			const match = $.root().html()?.match(/manga_id["']?\s*:\s*["']?(\d+)/);
			if (match?.[1]) {
		...
		async getDirectoryPath() {
			...
			const getPath = Application.getState(`dirpath_${this.domain}`);
			...
			const $ = load(Application.arrayBufferToUTF8String(buffer));
			const path = this.parser.parseDirectoryPath($, this);
...
//#region src/generic/config.ts
	...
	function customVersion(options) {
		...
		if (versionNumbers.length < 3) throw new Error(`Invalid BASE_VERSION: '${BASE_VERSION}'. Expected format: 'X.Y.Z' or 'X.Y.Z-prerelease.N'`);
		if ("increasePrerelease" in options) {
			...
			if (prereleaseParts.length < 2 || isNaN(Number(prereleaseParts[1]))) throw new Error(`Invalid prerelease format in BASE_VERSION: '${BASE_VERSION}'. Expected format: 'X.Y.Z-prerelease.N'`);
		...
		if (!(options.increaseMajor !== void 0 || options.increaseMinor !== void 0 || options.increasePatch !== void 0)) throw new Error("Empty options object provided. Either specify version increments or call getVersion() with no arguments.");
		return `${(versionNumbers[0] ?? 0) + (options.increaseMajor || 0)}.${(versionNumbers[1] ?? 0) + (options.increaseMinor || 0)}.${(versionNumbers[2] ?? 0) + (options.increasePatch || 0)}`;
	}

//#endregion
//#region src/Toonily/pbconfig.ts
	...
	pbConfig.contentRating = ContentRating$1.ADULT;

//#endregion
//#region src/Toonily/main.ts
	...
	var ToonilyExtension = class extends MadaraGeneric {
		...
		constructSearchRequest(page, query) {
			const urlBuilder = new URL$1(this.domain).addPathComponent(`search/${query?.title ? this.sanitizeQuery(query.title).replaceAll(" ", "-") + "/" : ""}page/${page.toString()}`).setQueryItem("post_type", "wp-manga");
			const genreFilters = Object.keys((query.metadata ?? []).find((x) => x.id === "genres")?.value ?? {});
			if (genreFilters.length) {
				genreFilters.forEach((genre, i) => urlBuilder.setQueryItem(`genre[${i}]`, genre));
				urlBuilder.setQueryItem("op", "1");
	...
	const Toonily = new ToonilyExtension();
...
//# sourceMappingURL=Toonily.js.map